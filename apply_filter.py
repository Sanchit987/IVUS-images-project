...
for i in range(NUMBER_OF_IMAGES):
    ...
    cv2.imwrite(os.path.join("Anisotropic Filter Images/", files[i][:-4] + ".png"), img_anisotropic)

    """ Applying wiener filter and saving it """
    img_wiener = wiener(grayscale)
    cv2.imwrite(os.path.join("Wiener Filter Images/", files[i][:-4] + ".png"), img_wiener)
    ...
    cv2.imwrite(os.path.join("Median Filter Images/", files[i][:-4] + ".png"), img_median)
    ...
    cv2.imwrite(os.path.join("Bilateral Filter Images/", files[i][:-4] + ".png"), img_bilateral)
    ...
    cv2.imwrite(os.path.join("Gaussian Filter Images/", files[i][:-4] + ".png"), img_gaussian)
    ...
    cv2.imwrite(os.path.join("Total Variance Filter Images/", files[i][:-4] + ".png"), img_denoise)


    """ Applying hybrid filters """
    # 1. Wiener + Anisotropic filter
    img_wiener_anisotropic = anisotropic_diffusion(img_wiener)
    cv2.imwrite(os.path.join("Wiener + Anisotropic Filter Images/", files[i][:-4] + ".png"), img_wiener_anisotropic)

    # 2. Non-local Means filter + Median filter
    img_ncm_median = median(img_ncm, disk(3), mode='constant', cval=0.0)
    cv2.imwrite(os.path.join("Non-local Means filter + Median Filter Images/", files[i][:-4] + ".png"), img_ncm_median)
